...
def main(config):
    ##################################################################
    # Initialization + Experimental Preparation
    ##################################################################
    svname = config['name']
    if svname is None:
    ...
    ###################################################################
    # Few-shot setting + Loading dataset
    ###################################################################
    ######################
    # 1.Few-shot setting
    #######################
    ...
    ###################################################################
    # Model settings + Optimizer settings
    ###################################################################
    ######################
    # 1.Model settings
    ######################
    ...
    ####################################
    # 2. Model training + model testing
    ####################################
    for epoch in range(cur_epoch, max_epoch + 1 + 1):
        ...
        for iteration in range(config['train_batches']):
            ############################################################
            # 0. Load Batch Samples for Classification and MetaLearning
            ############################################################
            if config.get('train_dataset'):
            ...
            #############################################################
            # 1. Classification for Branch 1 of Constellation in LCN-4
            #############################################################
            loss_list = []
            ...
            ##############################################################
            # 2. Branch 2 of Constellation of Constellation in LCN-4
            ##############################################################
        ...
        ##########################################################################
        # 2.2 Evaluation LCN-4（验证）
        ########################################################################## 
        # 1.Evaluation Mode for Classification 
        if eval_val:
            model.eval()
            for data, label in tqdm(val_loader, desc='val', leave=False):
                ...
                aves['va'].add(acc)
        # 2.Evaluation Mode for MetaLearning
        if eval_fs:
            model.eval()
            np.random.seed(0)
