...
# CaD: Differentiable Canberra Distance)
def diff_canberra_distance(qry, sup):
...
def diff_bray_curtis_distance(qry, sup):
    ...
    dcd_distance = F.normalize(dbcd_distance, p=2, dim=2)
    similarity = 1/(1 + dbcd_distance) 
    similarity = similarity[:, :, :sup_fea.size(1)]
    return similarity
...
def metametric(qry, sup):
    ...
    numerator = torch.abs(qry_fea_expand - sup_fea_expand)
    denominator = torch.abs(qry_fea_expand) + torch.abs(sup_fea_expand) + 1e-15  # 避免除以0的情况
    canberra_distance = torch.sum(numerator / denominator, dim=2)
            ...
            with torch.no_grad():           
                ...
                for method_i, logits_coeff, feat_source, branch in zip(method_i_list, logits_coeff_list, feat_source_list, branch_list):
                    ...
                    if feat_source == 'final':
                        ...
                        #########################################################################
                        # 3. Process embeddings & Calculate logits with proper distance function
                        #########################################################################
                        if method_i == 'cos':
